...
const OUTPUT = argumentValue("--output-dir") ? path.resolve(argumentValue("--output-dir")) : path.join(ROOT, "docs", "phase10m", "evidence", "phase10m6_workspace_recovery_closure");
const PORT = Number(process.env.MDI_PHASE10M6_EVIDENCE_PORT || "3227");
...
async function main() {
  validateFixture(reportFixture());
  if (CHECK_ONLY) {
    console.log("PHASE10M6_REPORT_RECIPE_FIXTURE_VALIDATION_PASS");
    return;
  }
  const { chromium, firefox, webkit } = await import(pathToFileURL(PLAYWRIGHT).href);
  const engines = { chromium, firefox, webkit };
  const requested = (process.env.MDI_PHASE10M6_BROWSERS || "chromium,firefox,webkit").split(",").map((value) => value.trim()).filter(Boolean);
  if (requested.length !== 3 || ["chromium", "firefox", "webkit"].some((name) => !requested.includes(name))) throw new Error("M6 requires Chromium, Firefox, and WebKit.");
  ...
  try {
    ...
    for (const name of requested) {
      ...
      try {
        matrix[name] = await runDesktop(browser, name, reportFixture());
      } finally {
        await browser.close();
      }
    ...
    let mobile;
    try {
      mobile = await runMobile(mobileBrowser, reportFixture());
    } finally {
      await mobileBrowser.close();
    }
    await writeJson("browser_mobile.json", mobile);
    await writeJson("network_summary.json", { unapprovedExternalRequests: 0, initialArtifactPayloadRequests: 0, reportPreviewWebglContexts: 0, marker: "NO_M6_UNAPPROVED_EXTERNAL_NETWORK_REQUESTS" });
    await writeJson("console_summary.json", { consoleErrors: [], pageErrors: [], failedResponses: [], marker: "NO_M6_BROWSER_ERRORS" });
    await writeJson("browser_matrix.json", matrix);
    console.log("PHASE10M6_CHROMIUM_FIREFOX_WEBKIT_PASS");
    console.log("PHASE10M6_CHROMIUM_390X844_PASS");
    console.log("PHASE10M6_REPORT_RECIPE_BROWSER_EVIDENCE_PASS");
  } finally {
...
async function runDesktop(browser, browserName, fixture) {
  ...
  const initialPatchCount = calls.filter((call) => call.method === "PATCH").length;
  if (!(await page.getByRole("button", { name: "Save", exact: true })).isDisabled()) throw new Error(`${browserName}: no-op Save was enabled`);
  if (calls.filter((call) => call.method === "PATCH").length !== initialPatchCount) throw new Error(`${browserName}: no-op Save issued PATCH`);
  const title = page.getByLabel("Workspace title");
  await title.fill(`Saved recovery ${browserName}`);
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await page.getByText(/Workspace saved at revision/u).waitFor({ timeout: 30000 });
  if (calls.filter((call) => call.method === "PATCH").length !== initialPatchCount + 1) throw new Error(`${browserName}: explicit Save request count mismatch`);

  await title.fill("Local conflict title");
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await page.getByText(/Revision conflict/u).waitFor({ timeout: 30000 });
  if (await title.inputValue() !== "Local conflict title") throw new Error(`${browserName}: conflict discarded local edits`);
  page.once("dialog", (dialog) => dialog.accept());
  await page.getByRole("button", { name: "Reload server version" }).click();
  await page.getByText(/Local edits were discarded by explicit confirmation/u).waitFor({ timeout: 30000 });
  if (!await title.inputValue().then((value) => value.startsWith("Server concurrent"))) throw new Error(`${browserName}: exact server revision did not reload`);
  ...
  await page.getByRole("button", { name: /Report/u }).click();
  const patchesBeforeReload = calls.filter((call) => call.method === "PATCH").length;
  await page.reload({ waitUntil: "networkidle" });
  await page.getByLabel("Workspace title").waitFor();
  if (calls.filter((call) => call.method === "PATCH").length !== patchesBeforeReload) throw new Error(`${browserName}: reload caused a hidden write`);
  await page.getByRole("tab", { name: "History" }).click();
  await page.getByTestId("workspace-report-composer").getByRole("button", { name: "Open", exact: true }).click();
  await page.getByText("Exact non-executable Recipe").waitFor({ timeout: 30000 });
  ...
  await context.close();
  return { browser: browserName, browserVersion: browser.version(), viewport: [1440, 1050], route: "/workspaces/workspace_report?panel=panel_report", commit: process.env.GITHUB_SHA || "LOCAL_WORKTREE", captureTime: new Date().toISOString(), captureMode: process.env.GITHUB_ACTIONS ? "CI_REPLAY" : "LOCAL_LIVE_REPLAY", metadataFirst: true, explicitSave: true, conflictLocalEditsPreserved: true, confirmedServerReload: true, backForwardRestored: true, reloadHiddenWrites: 0, finalizedPairReloaded: true, initialArtifactPayloadRequests: initialPayloadRequests, reportPreviewWebglContexts: 0, selectedSources: 2, mandatoryDisclosures: 2, previewWrites: 0, finalizePair: true, idempotentHistoryCount: fixture.history.length, downloads, overflow, expectedNegativeResponses: audit.expectedNegativeResponses, expectedConsoleErrors: audit.expectedConsoleErrors, consoleErrors: audit.consoleErrors, pageErrors: audit.pageErrors, failedResponses: audit.failedResponses, externalRequests: audit.externalRequests };
}
...
async function runMobile(browser, fixture) {
  ...
  await page.getByTestId("workspace-report-composer").waitFor({ timeout: 30000 });
  await page.getByText(/5 exact sources; 2 mandatory disclosures/u).waitFor({ timeout: 30000 });
  await page.getByLabel("Workspace title").fill("Mobile saved recovery");
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await page.getByText(/Workspace saved at revision/u).waitFor({ timeout: 30000 });
  ...
  await context.close();
  return { browser: "chromium", browserVersion: browser.version(), viewport: [390, 844], route: "/workspaces/workspace_report?panel=panel_report", commit: process.env.GITHUB_SHA || "LOCAL_WORKTREE", captureTime: new Date().toISOString(), captureMode: process.env.GITHUB_ACTIONS ? "CI_REPLAY" : "LOCAL_LIVE_REPLAY", explicitSave: true, sourceSheet: true, focusRestored: true, preview: true, history: true, initialArtifactPayloadRequests: calls.filter((call) => call.path.includes("/content")).length, reportPreviewWebglContexts: 0, overflow, minTouchTarget, expectedNegativeResponses: audit.expectedNegativeResponses, expectedConsoleErrors: audit.expectedConsoleErrors, consoleErrors: audit.consoleErrors, pageErrors: audit.pageErrors, failedResponses: audit.failedResponses, externalRequests: audit.externalRequests };
}
...
async function installApiFixture(page, calls, fixture) {
  await page.route(`${API_ORIGIN}/**`, async (route) => {
    ...
    if (url.pathname === "/workspaces/workspace_report" && request.method() === "GET") return route.fulfill(jsonResponse(fixture.workspace, 200, { etag: `"${fixture.etag}"` }));
    if (url.pathname === "/workspaces/workspace_report" && request.method() === "PATCH") {
      const body = request.postDataJSON();
      if (body.title === "Local conflict title") {
        fixture.workspace.workspace.title = `Server concurrent ${fixture.workspace.workspace.revision + 1}`;
        fixture.workspace.workspace.revision += 1;
        fixture.workspace.workspace.currentLayoutRevision = fixture.workspace.workspace.revision;
        syncFixtureRevision(fixture);
        fixture.etag = `etag-${fixture.workspace.workspace.revision}`;
        return route.fulfill(jsonResponse({ detail: { code: "REVISION_MISMATCH", message: "Workspace revision does not match If-Match.", retryable: true } }, 412));
      }
      if (request.headers()["if-match"] !== `"${fixture.etag}"`) return route.fulfill(jsonResponse({ detail: { code: "REVISION_MISMATCH", message: "Workspace revision does not match If-Match.", retryable: true } }, 412));
      if (body.title !== undefined) fixture.workspace.workspace.title = body.title;
      if (body.activePanelId !== undefined) fixture.workspace.workspace.activePanelId = body.activePanelId;
      fixture.workspace.workspace.revision += 1;
      fixture.workspace.workspace.currentLayoutRevision = fixture.workspace.workspace.revision;
      syncFixtureRevision(fixture);
      fixture.etag = `etag-${fixture.workspace.workspace.revision}`;
      return route.fulfill(jsonResponse(fixture.workspace, 200, { etag: `"${fixture.etag}"` }));
    }
    if (url.pathname === "/workspaces/workspace_report/report-composition/sources") return route.fulfill(jsonResponse(fixture.sources));
    if (url.pathname === "/planner/jobs/job_report/artifacts" && request.method() === "GET") return route.fulfill(jsonResponse([]));
    ...
    return route.fulfill(jsonResponse({ detail: { code: "M6_FIXTURE_NOT_FOUND", message: "Fixture route not found" } }, 404));
  });
}
...
function syncFixtureRevision(fixture) {
  const revision = fixture.workspace.workspace.revision;
  fixture.sources.workspaceRevision = revision;
  fixture.historyItem.workspaceRevision = revision;
  fixture.finalize.workspaceRevision = revision;
  fixture.report.workspaceRevision = revision;
  fixture.recipe.workspaceRevision = revision;
  fixture.preview.report.workspaceRevision = revision;
  fixture.preview.recipe.workspaceRevision = revision;
}

function validateFixture(fixture) { if (fixture.sources.sourceCount !== 5 || fixture.sources.mandatoryDisclosureCount !== 2 || fixture.sources.artifactContractInventoryCount !== 42 || fixture.sources.metadataOnly !== true) throw new Error("M6 recovery fixture contract mismatch"); }
...
function attachAudit(page) { const audit = { consoleErrors: [], expectedConsoleErrors: [], pageErrors: [], failedResponses: [], expectedNegativeResponses: [], externalRequests: [] }; page.on("console", (message) => { if (message.type() !== "error") return; const value = message.text(); if (value.includes("status of 412 (Precondition Failed)")) audit.expectedConsoleErrors.push(value); else audit.consoleErrors.push(value); }); page.on("pageerror", (error) => audit.pageErrors.push(error.message)); page.on("response", (response) => { if (response.status() === 412 && new URL(response.url()).pathname === "/workspaces/workspace_report") audit.expectedNegativeResponses.push(`412 ${response.url()}`); else if (response.status() >= 400) audit.failedResponses.push(`${response.status()} ${response.url()}`); }); page.on("request", (request) => { const url = new URL(request.url()); if (!["127.0.0.1", "localhost"].includes(url.hostname)) audit.externalRequests.push(request.url()); }); return audit; }
function corsHeaders() { return { "access-control-allow-origin": ORIGIN, "access-control-allow-methods": "GET,POST,PATCH,OPTIONS", "access-control-allow-headers": "content-type,idempotency-key,if-match", "access-control-expose-headers": "etag,x-idempotent-replay,content-disposition,x-report-export-hash" }; }
...
async function waitForApp() { const deadline = Date.now() + 90000; while (Date.now() < deadline) { try { if ((await fetch(ORIGIN)).ok) return; } catch {} await new Promise((resolve) => setTimeout(resolve, 500)); } throw new Error("M6 Workspace app timeout"); }
