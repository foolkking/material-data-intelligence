...
const EVIDENCE = path.join(ROOT, "docs", "phase10m", "evidence", "phase10m5_scientific_report_recipe");

async function main() {
  await mkdir(EVIDENCE, { recursive: true });
  const files = (await walk(EVIDENCE)).filter((item) => item !== "manifest.json").sort();
  const entries = [];
  ...
  const manifest = {
    schemaVersion: "phase10m5.evidence_manifest.v1",
    rule: "LF-normalized UTF-8 text; raw PNG bytes",
    entries,
  };
  await writeFile(path.join(EVIDENCE, "manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
  console.log(`PHASE10M5_EVIDENCE_MANIFEST_PASS files=${entries.length}`);
}
