...
export function BrillouinZoneSurface({bundle,capabilityOverride,engineFactory=defaultEngineFactory,summary,recipe}:BrillouinZoneSurfaceProps) {
  ...
  const onSelection=useCallback((next:BZSelection|null)=>{setSelection(next);setAnnouncement(next?`Selected ${next.kind} ${next.id}.`:"Selection cleared.");},[]);
  ...
  const updateVisibility=(key:keyof BZVisibility)=>{const next=Object.freeze({...visibility,[key]:!visibility[key]});setVisibility(next);engineRef.current?.setVisibility(next);if(selection&&((selection.kind==="point"&&!next.points)||(selection.kind==="vertex"&&!next.vertices)||(selection.kind==="face"&&!next.faces)||(selection.kind==="segment"&&!next.path)))setSelection(null);refreshSnapshot();};
