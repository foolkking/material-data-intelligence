...
import { buildElfOrbitalProduct, detectElfOrbitalProduct, validateElfOrbitalValues, type ElfOrbitalProduct } from "./elfOrbitalProduct";
...
export function VolumetricIsosurfaceSurface({ artifacts, capabilityOverride, byteLoader, workerFactory }: Props) {
  ...
  const [elfOrbitalProduct, setElfOrbitalProduct] = useState<ElfOrbitalProduct | null>(null);
  const [overlaySupercellRepeat, setOverlaySupercellRepeat] = useState<1 | 2>(1);
  ...
  const overlaySupercell = useMemo(() => deriveVolumetricOverlaySupercell(overlay, overlaySupercellRepeat), [overlay, overlaySupercellRepeat]);
  ...
  useEffect(() => {
    ...
    setMeshes([]); setPick(null); setAtomPick(null); setMetrics(null); setProduct(null); setPotentialProduct(null); setElfOrbitalProduct(null); setOverlaySupercellRepeat(1); setPotentialValues(null); setPotentialRawProfiles(null); setPotentialPoints([null, null]); setPotentialGauge("source_native"); setSelectedProfileIndex(null); setHoveredProfileIndex(null); setProfileWindow([0,1]); setSymmetricThreshold(true);
    ...
    void (async () => {
      try {
        ...
        const mappedElfOrbital = buildElfOrbitalProduct(validation.bundle);
        const selected = validation.bundle.fields.find((item) => item.supported && item.field.fieldId === (mappedPotential.fieldId ?? mappedElfOrbital.sourceFieldId ?? mappedProduct.defaultFieldId)) ?? getSupportedField(validation.bundle.fields);
        ...
        setBundle(validation.bundle); setProduct(mappedProduct); setPotentialProduct(mappedPotential); setElfOrbitalProduct(mappedElfOrbital); setActiveField(selected); setFieldId(selected.field.fieldId);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (["electron_localization_function", "orbital_density"].includes(activeField.field.quantity)) {
          const values = decodePotentialValues(decoded.buffer.slice(0), activeField.payload.dtype);
          const kind = activeField.field.quantity === "electron_localization_function" ? "elf" : "orbital_density";
          const range = validateElfOrbitalValues(values, activeField.payload.dtype, kind, activeField.field.contentHash);
          setElfOrbitalProduct(buildElfOrbitalProduct(bundle, range, activeField.field.fieldId));
        }
  ...
  }, [meshes, bundle, activeField, overlaySupercell]);
  ...
  const selectField = (nextFieldId: string) => {
    ...
    if (bundle) setElfOrbitalProduct(buildElfOrbitalProduct(bundle, null, nextFieldId));
    const nextLayers = defaultLayers(selected);
    setLayers(nextLayers); setLayerVisibility(Object.fromEntries(nextLayers.map((layer) => [layer.layerId, true])));
  };
  ...
  return <section className="viewer-renderer-surface volumetric-isosurface-surface" aria-label="Interactive volumetric isosurface renderer" data-testid="volumetric-isosurface-surface">
    ...
    {elfOrbitalProduct && elfOrbitalProduct.status !== "unavailable" && activeField && bundle && ["electron_localization_function", "orbital_density"].includes(activeField.field.quantity) ? <section className="volumetric-product-panel elf-orbital-product-panel" aria-label={`${elfOrbitalProduct.title} scientific summary`} data-testid="elf-orbital-product">
      <div data-testid="elf-orbital-product-header"><strong>{elfOrbitalProduct.title}</strong><span role="status" aria-live="polite">{elfOrbitalProduct.status}</span></div>
      <dl className="mini-grid" data-testid="elf-orbital-identity-summary"><dt>product</dt><dd>{elfOrbitalProduct.kind}</dd><dt>quantity</dt><dd data-testid="elf-orbital-quantity">{elfOrbitalProduct.quantity}</dd><dt>unit</dt><dd data-testid="elf-orbital-unit">{elfOrbitalProduct.unit}</dd><dt>source format</dt><dd>{bundle.sourceFormat}</dd><dt>parser</dt><dd>{activeField.field.provenance.producer} {activeField.field.provenance.producerVersion}</dd><dt>source field</dt><dd>{elfOrbitalProduct.sourceFieldId}</dd><dt>source field hash</dt><dd>{elfOrbitalProduct.sourceFieldHash}</dd><dt>normalization</dt><dd>{elfOrbitalProduct.normalizationSemantics}</dd><dt>integral semantics</dt><dd>{elfOrbitalProduct.integralSemantics}</dd>{elfOrbitalProduct.identity ? <><dt>identity completeness</dt><dd data-testid="orbital-identity-completeness">{elfOrbitalProduct.identity.completeness}</dd><dt>source identity</dt><dd>{elfOrbitalProduct.identity.displayName}</dd><dt>band / k-point / occupancy</dt><dd>unavailable / unavailable / unavailable</dd></> : null}</dl>
      <div className="viewer-renderer-controls" aria-label={`${elfOrbitalProduct.title} contour presets`} data-testid="elf-orbital-presets">{elfOrbitalProduct.presets.map((preset) => <button type="button" key={preset.id} onClick={() => applyPreset(preset.exactIsovalue)}>{preset.label} ({format(preset.exactIsovalue)} {preset.unit})</button>)}</div>
      <div className="viewer-renderer-controls" data-testid="elf-orbital-supercell"><label>Structure context <select aria-label="Structure overlay supercell" value={overlaySupercellRepeat} disabled={overlay?.kind !== "periodic_viewer_scene"} onChange={(event) => { setPick(null); setAtomPick(null); setOverlaySupercellRepeat(Number(event.target.value) === 2 ? 2 : 1); }}><option value="1">1×1×1 source cell</option><option value="2">2×2×2 bounded overlay</option></select></label><span data-testid="elf-orbital-supercell-status">{overlaySupercell.status}; {overlaySupercell.replicas} replica(s); {overlaySupercell.atomCount} atoms; {overlaySupercell.bondCount} bonds. Scalar field remains the immutable source cell.</span></div>
      <table data-testid="elf-orbital-statistics"><caption>Source field statistics and validation</caption><thead><tr><th>Measure</th><th>Value</th><th>Authority</th></tr></thead><tbody><tr><td>minimum</td><td>{format(activeField.field.minimum)}</td><td>canonical payload statistics</td></tr><tr><td>maximum</td><td>{format(activeField.field.maximum)}</td><td>canonical payload statistics</td></tr><tr><td>mean</td><td>{format(activeField.field.mean)}</td><td>canonical payload statistics</td></tr><tr><td>standard deviation</td><td>{format(activeField.field.standardDeviation)}</td><td>canonical payload statistics</td></tr><tr><td>RMS</td><td>{format(activeField.field.rms)}</td><td>canonical payload statistics</td></tr><tr><td>full-cell integral</td><td>{elfOrbitalProduct.integral === null ? "unavailable" : `${format(elfOrbitalProduct.integral)} ${elfOrbitalProduct.integralUnit}`}</td><td>{elfOrbitalProduct.integralInterpretation}</td></tr><tr><td>range status</td><td data-testid="elf-orbital-range-status">{elfOrbitalProduct.rangeValidation?.status ?? "validating"}</td><td>{elfOrbitalProduct.rangeValidation?.tolerancePolicy ?? "decoded payload pending"}</td></tr><tr><td>{elfOrbitalProduct.kind === "elf" ? "below zero / above one" : "negative samples"}</td><td>{elfOrbitalProduct.rangeValidation ? elfOrbitalProduct.kind === "elf" ? `${elfOrbitalProduct.rangeValidation.belowZeroCount} / ${elfOrbitalProduct.rangeValidation.aboveOneCount}` : elfOrbitalProduct.rangeValidation.negativeCount : "validating"}</td><td>decoded source values; no clamping</td></tr></tbody></table>
      {elfOrbitalProduct.warnings.length ? <ul className="warning" data-testid="elf-orbital-warnings">{elfOrbitalProduct.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul> : null}
      {elfOrbitalProduct.kind === "elf" ? <p className="notice" data-testid="elf-interpretation-boundary">ELF isosurfaces show the spatial set at the selected ELF value. They do not automatically identify chemical bonds, lone pairs, electron basins, or atomic shells.</p> : <p className="notice" data-testid="orbital-interpretation-boundary">This is source-defined partial density. No single-orbital, HOMO/LUMO, occupancy, probability, bonding, or orbital-character identity is inferred.</p>}
      <p className="empty-state">Source values are immutable: no clamp, absolute value, square, smoothing, rescaling, or renormalization is applied. Signed real orbital amplitude and complex wavefunction products are deferred by design.</p>
    </section> : null}
    {product && bundle && potentialProduct?.status !== "ready" && elfOrbitalProduct?.status === "unavailable" ? <section className="volumetric-product-panel" aria-label="Charge and spin density scientific summary" data-testid="charge-spin-density-product">
    ...
    {pick ? <p className="notice" data-testid="volumetric-surface-inspector">Surface {pick.layerId}, {pickedPotentialLayer ? <>source isovalue {format(pickedPotentialLayer.sourceIsovalue)} / displayed isovalue {format(pickedPotentialLayer.displayedIsovalue)} {pickedPotentialLayer.unit}</> : <>isovalue {pick.isovalue} {activeField?.field.unit}</>}, {elfOrbitalProduct && elfOrbitalProduct.status !== "unavailable" ? <>quantity {elfOrbitalProduct.quantity}, field {elfOrbitalProduct.sourceFieldHash}, </> : null}triangle {pick.triangleIndex}, position {pick.cartesianPosition.map((value) => value.toFixed(3)).join(", ")}, mesh {pick.meshHash}</p> : null}
...
function defaultLayers(field:VolumetricFieldCompatibility):IsosurfaceLayerRequest[]{const range=Math.max(Math.abs(field.field.minimum),Math.abs(field.field.maximum));const span=field.field.maximum-field.field.minimum;const potential=["local_potential","electrostatic_potential"].includes(field.field.quantity);const elf=field.field.quantity==="electron_localization_function";const orbital=field.field.quantity==="orbital_density";const signed=!potential&&!elf&&!orbital&&(field.field.spin?.channel==="spin_difference"||(field.field.minimum<0&&field.field.maximum>0));const value=elf?0.7:orbital?Math.max(0,field.field.maximum)*0.25:potential?field.field.mean:field.field.minimum+span*0.25;return signed?pairedLayers(range*0.25):[{layerId:"surface-1",isovalue:value,sign:value<0?"negative":"positive"}];}
