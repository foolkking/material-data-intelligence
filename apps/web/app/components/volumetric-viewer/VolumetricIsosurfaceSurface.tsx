...
export function VolumetricIsosurfaceSurface({ artifacts, capabilityOverride, byteLoader, workerFactory }: Props) {
  ...
  useEffect(() => {
    ...
    setMeshes([]); setPick(null); setAtomPick(null); setMetrics(null);
    ...
    void (async () => {
      try {
        ...
        const selected = getSupportedField(validation.bundle.fields);
        ...
        setBundle(validation.bundle); setActiveField(selected); setFieldId(selected.field.fieldId);
        setOverlay(mappedOverlay && mappedOverlay.ok ? mappedOverlay.overlay : null);
        const range = Math.max(Math.abs(selected.field.minimum), Math.abs(selected.field.maximum));
        const span = selected.field.maximum - selected.field.minimum;
        const nextLayers: IsosurfaceLayerRequest[] = selected.field.minimum < 0 && selected.field.maximum > 0
          ? [{ layerId: "positive-1", isovalue: Math.max(range * 0.25, Number.EPSILON), sign: "positive" }, { layerId: "negative-1", isovalue: -Math.max(range * 0.25, Number.EPSILON), sign: "negative" }]
          : [{ layerId: "surface-1", isovalue: selected.field.minimum + span * 0.25, sign: selected.field.minimum >= 0 ? "positive" : "negative" }];
  ...
  const updateLayer = (layerId: string, value: number) => setLayers((current) => current.map((layer) => layer.layerId === layerId ? { ...layer, isovalue: value } : layer));
  ...
  return <section className="viewer-renderer-surface volumetric-isosurface-surface" aria-label="Interactive volumetric isosurface renderer" data-testid="volumetric-isosurface-surface">
    ...
    {bundle && activeField ? <div className="viewer-preview-grid"><div><dl className="mini-grid"><dt>source</dt><dd>{bundle.sourceFormat}</dd><dt>field</dt><dd>{activeField.field.fieldName}</dd><dt>unit</dt><dd>{activeField.field.unit}</dd><dt>grid</dt><dd>{bundle.grid.shape.join(" x ")} / {bundle.grid.periodic ? "periodic" : "affine"}</dd><dt>range</dt><dd>{activeField.field.minimum} to {activeField.field.maximum}</dd></dl></div><div><label>Field <select data-testid="volumetric-field-selector" value={fieldId} onChange={(event) => selectField(event.target.value)}>{bundle.fields.map((item) => <option key={item.field.fieldId} value={item.field.fieldId} disabled={!item.supported}>{item.field.fieldName}{item.supported ? "" : " (unsupported)"}</option>)}</select></label><label>Projection <select value={projection} onChange={(event) => setProjection(event.target.value as typeof projection)}><option value="perspective">Perspective</option><option value="orthographic">Orthographic</option></select></label><label>Opacity <input type="range" min="0.1" max="0.9" step="0.01" value={opacity} onChange={(event) => setOpacity(Number(event.target.value))} /></label></div></div> : null}
