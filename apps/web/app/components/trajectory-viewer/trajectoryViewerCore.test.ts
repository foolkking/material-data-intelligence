...
describe("trajectory viewer core",()=>{
  ...
  it("evicts mapped frames before insertion and protects the committed frame",()=>{const trajectory=validated(fixed);const cache=new TrajectoryFrameCache(2,4096,trajectory.trajectory_id);cache.set(mapTrajectoryFrame(trajectory,0));cache.set(mapTrajectoryFrame(trajectory,1));expect(cache.get(0)?.frameIndex).toBe(0);cache.set(mapTrajectoryFrame(trajectory,2),0);expect(cache.snapshot()).toMatchObject({indices:[0,2],evictions:1,peakFrames:2});expect(cache.snapshot().peakBytes).toBeLessThanOrEqual(4096);expect(cache.get(1)).toBeUndefined();cache.clear();expect(cache.snapshot().size).toBe(0);});
});
