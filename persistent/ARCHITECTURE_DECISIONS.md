...
Phase 10F-12 implemented the canonical `structure.viewer_scene` adapter and
proved registry, PlanValidator, QueueWorkerRuntime, artifact generation, and
frontend regression coverage. The remaining evidence gap was real Chrome
preview evidence using artifacts generated by a live adapter job rather than
static fixtures.

### Decision

Phase 10F-13 adds evidence tooling only:

- a Python generator captures live planner/job/runtime execution for
  `structure.viewer_scene`;
- a real Chrome runner opens the existing JSON-only preview surface with those
  captured live API responses;
- screenshots, DOM, console, network, API transcript, artifact audit, security
  audit, and schema compatibility evidence are stored under Phase 10F-13
  evidence docs.

No production API route, renderer, dependency, or Tool Registry semantic change
is added.

### Consequences

- Adapter-generated `viewer_scene.v1` artifacts now have live browser/API
  evidence.
- Old Phase 10D viewer metadata/export schemas remain unchanged and documented
  as compatibility debt.
- Renderer handoff evidence is ready, but renderer API shape, dependency
  selection, sandbox/security plan, and implementation remain future work.
- Full `structure.viewer_3d`, WebGL, Three.js, MatterViz, renderer bundles,
  Brillouin-zone 3D, phonon, notebook/script execution, external APIs, and real
  LLM execution remain out of scope.
