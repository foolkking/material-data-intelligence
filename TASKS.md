...
* `packages/adapters/mdi_adapters/platform_builtin/brillouin_zone.py`、adapter registry/export、Tool Registry manifest/loader和 planner provider。
* `packages/artifact-core/mdi_artifact_core/brillouin_zone_contract.py`、Python/JSON/TypeScript artifact type declarations。
* `apps/web/app/components/PlannerWorkbench.tsx`及其测试。
* `tests/test_phase10i1_brillouin_zone_adapter.py`、Phase 10I contract/manifest regression tests。
* `scripts/generate_phase10i1_brillouin_zone_adapter_evidence.py`、`docs/phase10i/evidence/phase10i1_brillouin_zone_adapter/`、Phase 10I-1 docs 与 persistent 记录。

修改摘要：实现正式 JSON-only `structure.brillouin_zone` adapter，包含单个有序非磁性 3D 结构边界、primitive standardization、physics-`2*pi` reciprocal lattice、bounded first-BZ Wigner-Seitz geometry、Setyawan-Curtarolo k-path、六类 inert artifacts、Tool Registry/Planner/PlanValidator/QueueWorkerRuntime 闭环、application-owned JSON preview 和 sanitized deterministic evidence。未实现 Three.js/WebGL renderer、GPU/browser BZ product、electronic/phonon bands、external network、artifact code 或 real LLM。

测试结果：Phase 10I/10I-1/registry focused `88 passed`；frontend full `194 passed`；backend full `648 passed, 23 skipped, 56 warnings`；typecheck、production build、`uv lock --check`、Ruff、`git diff --check` 通过；Phase 10 closure Chromium/Firefox/WebKit、mobile/accessibility/performance/integrity regression markers 通过；`BRILLOUIN_ZONE_ADAPTER_EVIDENCE_PASS`、`BRILLOUIN_ZONE_RUNTIME_EVIDENCE_PASS`、`NO_BRILLOUIN_ADAPTER_EXTERNAL_NETWORK_REQUESTS`、`NO_SECRET_PATTERN_HITS`。本机无 Docker CLI，service-backed 与 no-skipped 由 current-HEAD CI 通过；`npm audit` 因配置的 npmmirror audit endpoint 返回 `404 NOT_IMPLEMENTED` 不可用，本任务无 dependency/lockfile 变更。

提交 / CI：implementation commit `08d7742ddc6d1574a79c99baf90f019f3635aa3f`；CI run `29384696711` success；unit、frontend typecheck/build、service-backed integration 及 no-skipped assertion 均通过。Completion record 提交及其 current-HEAD CI 仍是队列归档前置条件。
