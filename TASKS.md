...
这是 Phase 10J-6 归档和 Gate J6-R Product Roadmap Correction 完成后的第一个正式新阶段。

本阶段属于：

**Material Intelligence Layer 的真实仓库能力审计与实施边界冻结。**

本阶段不是实现阶段。

本阶段的最终目标是回答：

> 当前 Material Data Intelligence 仓库距离“材料数据智能分析层”到底还缺什么？

必须基于真实代码、真实 schema、真实 Tool Registry、真实 DataProfile、真实 Planner、真实前端和真实测试回答。

不要根据旧 planning 文档猜。

不要因为某个 pymatviz 函数存在就写成平台已经支持。

不要实现 Phase 10K-1。

---

# 0. 当前 Canonical Baseline
...
Phase 10J-6 implementation：

`9cd0c693881f13be6ef0b0068563ec26a3b3d82e`

Gate J6-R roadmap correction commit：

`55ab8a956b3f3efa3e8dffcc6f87d0f2915d9680`
...
Phase 11
Scientific Validation & Honest Coverage
...
现在用户已经明确批准进入 Phase 10K-0。

因此本阶段可以进入 executable queue。

---

# 1. Canonical Product Goal
...
* phonopy
* Plotly
* application-owned Three.js/WebGL
* LLM Agent
* Tool Registry
* Adapter
* Artifact
...
**Material Data Profile + Dataset Intelligence + Materials ML Intelligence。**

Phase 10K 不解决：

* Agent orchestration → Phase 10L
* Unified Workspace → Phase 10M
* CrystalNN / Experimental XRD / Trajectory analytics / Electronic Band-DOS → Phase 10N

必须保持边界。

---

# 2. Phase 10K Canonical Subphase Direction

本阶段必须审计并验证以下后续划分是否符合真实仓库。

当前 reviewer 推荐：

```text
Phase 10K-0
Material Intelligence Capability Gap Audit
...
Phase 10K-0 必须根据真实代码验证：

* 是否存在重复能力
* 是否某能力已经 READY
* 是否需要合并
* 是否应该调整具体 implementation scope

但：

**不得推翻 Gate J6-R 已冻结的 Phase 10K 产品目标。**

只能细化 Phase 10K 内部实现范围。
...
* DataProfile audit
* parser/resource audit
* Planner behavior audit
* frontend audit
* existing test audit
* dependency audit
* official/example capability mapping audit
* documentation
* gap matrix
...
* 实现 DataProfile 2.0
* 修改 DataProfile schema
* 新增 parser
* 新增 adapter
* 新增 Tool Registry tool
* 实现 parity plot
* 实现 residual plot
* 实现 uncertainty analysis
* 实现 confusion matrix
* 实现 clustering
* 实现 PCA/UMAP
* 新增 Python dependency
* 新增 npm dependency
* 修改 Planner routing
* 修改 AnalysisPlan schema
* 修改 PlanValidator
* 修改 QueueWorkerRuntime
* 修改 `/planner/jobs` 主语义
* 修改 Artifact contract 主语义
* 修改 frontend product behavior
* 实现 Workspace
* 实现 multi-tool planning
* 实现 CrystalNN/VoronoiNN
* 实现 Experimental XRD
* 实现 trajectory analytics
* 实现 Electronic Band/DOS
* 实现 Fermi Surface
* 访问外部 API
* 执行 notebook
* 执行任意用户 script
...
* 加载外部资源
* 为了测试去安装 optional scientific dependencies
...
* repo = Material Data Intelligence
* branch = master
* Gate J6-R commit `55ab8a956b3f3efa3e8dffcc6f87d0f2915d9680` 存在于当前历史
* Phase 10J-6 已 ARCHIVED
* `docs/ROADMAP.md` 存在
* Roadmap 当前 authoritative
* Future Scope 与 Not Planned Scope 已存在或已有 canonical equivalent
* 工作区状态明确

如果当前工作区有未提交变更：

先判断是否属于用户本轮 Phase 10K-0 合法进入造成的 TASKS transition。
...
如果 TASKS 一次只保存 active block：

只加入 10K-0。

---

# 6. 必读 Canonical Documentation
...
* lightweight structure summaries

以及相关 browser/API evidence。
...
这些文档是历史 capability input。

**不是 current roadmap authority。**

---

# 8. Audit A — Current Data Resource / Parser Layer
...
```bash
rg -n "DataProfile|data_profile|profile|ResourceKind|resource_type|parser|parse_" backend packages tests
```

定位：

* DataProfile schema/model
...
# Current Material Input Capability Matrix

至少检查：

| Data Kind              | Parser Exists | Normalized Resource | Profile Recognition | Tool Consumption | Tests |
| ---------------------- | ------------- | ------------------- | ------------------- | ---------------- | ----- |
| CSV / table            |               |                     |                     |                  |       |
| JSON table             |               |                     |                     |                  |       |
| formula/composition    |               |                     |                     |                  |       |
...
不得因为 parser 能读取 CSV 就写：

“model prediction supported”

除非 profiler/tool 层真的识别相应语义。

---

# 9. Audit B — Current DataProfile
...
# Current DataProfile Field Inventory

逐字段记录：

| Field | Source | Deterministic | Persisted | Planner Uses It | Frontend Shows It | Scientific Meaning |
| ----- | ------ | ------------: | --------: | --------------: | ----------------: | ------------------ |
...
是否存在 semantic column classification。

## Scientific Property Recognition
...
* grid
* structure binding
* charge/spin/potential/ELF/orbital identity

然后给 DataProfile 当前成熟度：
...
# 10. Audit C — Semantic Column Detection

Material Intelligence 不能只依赖：
...
因此审计是否存在 semantic inference：

```bash
rg -n "y_true|y_pred|y_std|target|prediction|uncertainty|label|probability|formula|composition|band_gap|formation_energy" backend packages tests apps
```
...
这将直接决定 10K-1 的 implementation scope。

---

# 11. Audit D — Existing Table / Dataset Tools

完整审计当前 Tool Registry。

搜索：

```bash
rg -n "table\\.|viz\\.|composition\\." tool_registry backend packages tests
```

如果 registry 路径不同，以实际代码为准。

建立：

# Existing Dataset/Composition Tool Inventory
...
| Tool | Registered | Input | Params | Output | Planner Route | Browser Evidence | Current Product Role |
| ---- | ---------: | ----- | ------ | ------ | ------------: | ---------------: | -------------------- |
...
不要仅因为有 10 个 tools 就写 Dataset Explorer READY。

---

# 12. Audit E — Dataset-Level Material Statistics Gaps
...
* space-group distribution across dataset
* lattice a/b/c distribution
* lattice angle distribution
* volume distribution
...
* READY
* derivable but no formal product
* PARTIAL
* MISSING
...
“Dataset Materials Explorer 已实现”。

---

# 13. Audit F — Existing Materials ML Capability

这是 Phase 10K 的关键缺口审计。

搜索：

```bash
rg -n "parity|density_scatter|residual|MAE|RMSE|R2|r2_score|uncert|calibration|error_decay|confusion|ROC|precision_recall|classification|model comparison" backend packages apps tests docs
```
...
# Materials ML Capability Matrix

## Regression

| Capability               | Backend | Tool | Artifact | Frontend | Evidence | Status |
| ------------------------ | ------- | ---- | -------- | -------- | -------- | ------ |
| parity plot              |         |      |          |          |          |        |
| density scatter          |         |      |          |          |          |        |
| residual                 |         |      |          |          |          |        |
...
# 14. Audit G — Composition Embedding / Clustering

审计代码与 dependency：

```bash
rg -n "cluster_compositions|embedding|PCA|UMAP|TSNE|t-SNE|Magpie|matminer|Matscholar|MEGNet|sklearn|scikit" backend packages apps tests pyproject.toml uv.lock
```
...
* umap
* matminer
* optional pymatviz extras

哪些已经存在。

不要联网查版本。

只读 repository lock/pyproject。

## 10K-4 建议

必须基于现有依赖回答：

第一版 composition space 最合理的是：

* element-fraction + PCA？
* existing pymatviz cluster_compositions？
* optional embedding？
* another current dependency-backed method？

本阶段不实现。

---

# 15. Audit H — Pymatviz Capability Reuse

Phase 10K 不能重新造 pymatviz。

审计当前 installed pymatviz 以及已有 usage。

查找：

```bash
rg -n "pymatviz|pmv\\." backend packages tests
```

同时读取 capability inventory 中与以下有关部分：
...
# Library Reuse Decision Matrix

| Desired Product Capability | pymatviz | pymatgen | Plotly | Existing Internal Code | Recommended Reuse |
| -------------------------- | -------- | -------- | ------ | ---------------------- | ----------------- |
...
不要默认必须新增 pymatviz wrapper。

应该判断：

哪一层负责：

* scientific/statistical calculation
* visualization
* artifact representation
* linked sample inspection

原则：

> product capability first, library function second.

---

# 16. Audit I — Planner Current Use of DataProfile

Phase 10L 才实现 capability-aware Planner。

但 Phase 10K-0 必须弄清当前真实状态。

搜索：

```bash
rg -n "DataProfile|data_profile|profile_id|profile|tool_id|route|routing|planner" services backend packages tests
```
...
# Planner/Profile Integration Audit

| Signal             | Mock/Deterministic Planner Uses | LLM Planner Receives | PlanValidator Uses |
| ------------------ | ------------------------------: | -------------------: | -----------------: |
| resource kind      |                                 |                      |                    |
| column schema      |                                 |                      |                    |
| formula presence   |                                 |                      |                    |
...
# 17. Audit J — Current Frontend Dataset Experience
...
apps/web
PlannerWorkbench
dataset/profile related components
...
```bash
rg -n "DataProfile|Dataset|Profile|composition|histogram|correlation|prediction|uncertainty|model" apps/web
```
...
* rows/columns
* detected materials information
* formula
* structure
...
Tool execution 后只显示 artifact list？

## Model evaluation

是否有专门产品 UI？

## Cross-filter / sample inspection

是否存在？

输出：

# Current Dataset UX Maturity
...
Phase 10M 才做 Unified Workspace。

不要把整个 Workspace 缺口塞入 10K。

10K 只关注 dataset/material-intelligence experience。

---

# 18. Audit K — Artifact / Report Compatibility

检查当前 Artifact system 能否自然承载未来 10K capability。
...
* Plotly/static chart JSON
* row/sample table
* outlier list
* model metric summary
* composition embedding coordinates
* cluster labels
* dataset comparison summary

如果现有 artifact contract 足够：

明确写：

REUSE.

如果需要新的 scientific schema：
...
Recipe / Report 是否能够引用这些 future artifacts。

---

# 19. Audit L — Data Size / Performance Boundaries

Phase 10K 面向 dataset，数据规模风险与之前单结构不同。
...
这只是 planning-level recommendation。

不要修改 cap。

---

# 20. Audit M — Scientific/Statistical Dependency Boundary

检查现有 dependency 是否支持未来：
...
1. existing Python stdlib / numpy/scipy
2. current dependency
3. pymatviz/pymatgen existing stack
4. 最后才考虑新依赖
...
# 21. Official / Existing Example Mapping

不要正式进入 Phase 11 validation。

但为了避免 10K 重复设计，需要只读审计现有 official/example pack 中与 Material Intelligence 有关的 cases。
...
# Material Intelligence Example Mapping

| Capability | Existing Official/Benchmark Case | Current Platform Support | Future 10K Phase |
| ---------- | -------------------------------- | ------------------------ | ---------------- |
...
不得运行 notebook/script。

---

# 22. Product-Level Gap Classification

把所有发现统一分成：

## READY

正式 Tool/Adapter/product/evidence 已存在。
...
## MISSING_INITIAL_RELEASE

Phase 10K 必须实现。

## DEFER_TO_10L

属于 Agent 智能，不属于 Material Intelligence implementation。

## DEFER_TO_10M

属于 Unified Workspace。

## DEFER_TO_10N

属于 Professional Scientific Completion。
...
```text
docs/phase10k/phase10k0_material_intelligence_gap_matrix.md
```

包含：

| Capability | Existing Code | Existing Tool | Existing UI | Existing Evidence | Gap | Target Phase |
| ---------- | ------------- | ------------- | ----------- | ----------------- | --- | ------------ |
...
* phonon recognition
* volumetric recognition
* available-analysis discovery
...
# 24. Phase 10K Implementation Decision

本阶段最终必须给出 reviewer-ready implementation recommendation。
...
Planner behavior modification仍留给 10L。

10K-1 可以为 Planner 暴露 profile capability。

不能在 10K-1 实现 capability-aware planning。

---

## Phase 10K-2 — Dataset Materials Explorer

明确建议实现的产品 capability。

不要只列 pymatviz function names。
...
Regression / Uncertainty / Classification 中哪些。

必须采用：

data-driven capability gate。
...
只有有 class labels/probabilities：
...
并根据现有 dependency 决定第一批算法。

不要为了“完整支持 pymatviz 所有 embedding”扩大 scope。

---

## Phase 10K-5 — Integration / Evidence
...
而不只是 adapter unit tests。

---

# 25. 推荐 10K 设计原则

必须写入 Phase 10K planning docs。

### Principle 1

**Product capability > library wrapper**

不要：

`pymatviz function = one tool`

机械封装。

### Principle 2

**Profile drives capability**
...
**Deterministic scientific/statistical layer first**
...
**LLM interpretation later**

10K 只生成可靠 structured results。

10L 才让 Agent 自动组合/解释。

### Principle 5

**Dataset-level first-class resource**
...
未来 ML/outlier artifacts 必须能够引用原 dataset row/sample identity。

### Principle 7

**No arbitrary code**

继续通过 Tool Registry / Adapter。
...
```text
docs/phase10k/phase10k0_material_intelligence_capability_gap_audit.md
docs/phase10k/phase10k0_material_intelligence_gap_matrix.md
docs/phase10k/phase10k0_current_dataprofile_inventory.md
docs/phase10k/phase10k0_dataset_ml_capability_inventory.md
docs/phase10k/phase10k0_phase10k_implementation_sequence.md
docs/phase10k/phase10k1_next_scope.md
```
...
除非发现 Phase 10K 描述与 Gate J6-R frozen roadmap 有事实不一致。

本阶段没有权限重新改变战略 roadmap。

---

# 27. Persistent Updates
...
Material Intelligence capability audit completed。
...
* semantic column alias policy
* sample identity
* regression metric authority
* uncertainty representation
* composition embedding first-batch method
* dataset row cap
...
existing dataset/composition tools。

记录：

future 10K candidates。
...
只有出现真正需要 ADR 的新 architecture decision 才更新。

不要为了每个 Phase 都强行新增 ADR。

---

# 28. No Premature Tool Naming
...
不要每个 histogram 创建新 tool。

10K-0 必须评估：

**product-oriented tool granularity。**

---

# 29. Frontend Product Recommendation
...
要根据现有 PlannerWorkbench architecture 推荐最小集成方式。

不要在本阶段实现。

---

# 30. Test / Evidence Planning

为每个 10K implementation phase 定义未来测试。
...
* uncertainty
* classification
* missing-required-field typed errors
...
如果已有 Phase 10 evidence integrity regression：

运行。

不得重新跑所有重型浏览器 evidence，除非 current regression suite 本身要求。
...
所有 audit/docs/persistent 检查通过后：
...
```text
Audit material intelligence capability gaps
```
...
不得将 queued/in-progress 当 success。

---

# 34. Completion Record / Queue Archive

按照现有项目 Phase queue 纪律：

implementation/audit commit CI 通过后：

记录 Phase 10K-0 completion result。

然后 completion record commit。

验证 completion-record exact SHA current-head CI。
...
## 3. Current Material Input Capabilities
...
parser/profile/tool state。

## 4. Current DataProfile
...
READY / PARTIAL_READY / MINIMAL / MISSING
...
## 6. Existing Dataset / Composition Tools
...
tool collection vs product-ready explorer。

## 7. Dataset Intelligence Gap Matrix
...
## 10. Library Reuse Assessment

* pymatviz:
* pymatgen:
* Plotly:
* numpy/scipy:
* current ML/stat dependencies:
* optional dependency findings:

## 11. Planner / Profile Integration

* deterministic planner:
* LLM planner:
* resource awareness:
* profile awareness:
* column semantic awareness:
* current classification:

PROFILE_AWARE / PARTIAL_PROFILE_AWARE / MOSTLY_PROMPT_ROUTED / KEYWORD_ROUTED

## 12. Current Dataset UX
...
## 13. Artifact / Recipe / Report Readiness

* existing contracts reusable:
...
## 14. Data Size / Performance

* current caps:
* missing caps:
* recommended 10K boundaries:

## 15. Product Gap Classification
...
## 16. Phase 10K Implementation Decision
...
Material Intelligence Integration / Evidence

同上。

## 17. Recommended Tool Granularity

说明：

* which existing tools remain
* which product-level tools may be added
* which overly-fragmented tools should not be created
...
## 22. Whether Allowed to Enter Phase 10K-1
...
* DataProfile inventory complete
* dataset tool inventory complete
* ML audit complete
* embedding/dependency audit complete
* Planner/Profile integration audit complete
* frontend audit complete
* gap matrix complete
* 10K implementation scopes frozen
* no implementation accidentally started
* current-head CI success
* completion record CI success
* git clean
* origin/master == HEAD
...
1. 审计真实仓库，而不是仅根据 docs。
2. 完整识别当前 DataProfile。
3. 明确 DataProfile 对材料语义的真实识别范围。
4. 明确 model prediction/uncertainty/classification 当前状态。
5. 完整 inventory dataset/composition tools。
6. 不把一堆单 tools 误写成 Dataset Explorer READY。
7. 完整审计 Materials ML。
8. 完整审计 composition embedding/clustering。
9. 审计真实 dependency。
10. 审计 Planner 是否真正使用 Profile。
11. 审计 frontend dataset experience。
12. 审计 Artifact/Recipe compatibility。
13. 明确 sample identity 问题。
14. 明确 dataset size/caps 风险。
15. 输出正式 Gap Matrix。
16. 明确 10K-1～10K-5 implementation scope。
17. 不机械封装 pymatviz 函数。
18. 不新增 tool。
19. 不新增 schema。
20. 不修改 Planner。
21. 不修改 Runtime。
22. 不修改 frontend product code。
23. 不新增 dependency。
24. 不执行真实 LLM。
25. 不执行 notebook/script。
26. 不访问外部网络。
27. docs/persistent 更新准确。
28. tests/checks 通过。
29. exact-head CI success。
30. completion record CI success。
31. 10K-0 archived。
32. 10K-1 remains NEXT only。
33. git clean。
34. origin/master == HEAD。

如果审计发现当前仓库实际上已经实现了某个 10K planned capability：

不要重复实现。

将其标为：

`READY` 或 `REUSABLE_FOUNDATION`

并调整相应后续 scope。

如果审计发现某个既定 10K capability 与真实产品目标不符：

记录：

`REVIEWER_DECISION_REQUIRED`

不要自行删除 Gate J6-R 已批准的 Initial Release capability。
...
# Phase 10K-0 Repository / Material Intelligence State Review

不要修改实现代码。

先用真实仓库回答：

1. 当前 DataProfile 到底知道什么？
2. 当前 dataset/composition 工具到底能做什么？
3. 当前 Materials ML 到底实现了多少？
4. 当前 composition embedding/clustering 是否已经存在？
5. Planner 是否真正依据 DataProfile 决策？
6. 用户现在能否像分析“一批材料”一样使用这些能力？
7. 哪些是已经存在但尚未产品化的 foundation？
8. Phase 10K-1～10K-5 分别还需要真正实现什么？

完成审计后直接继续文档、Gap Matrix、persistent、commit 和 CI 闭环。
...
**Material Intelligence Layer 的第一个正式实现阶段。**

目标不是新增更多图表，也不是增强 LLM Planner。

目标是把当前 DataProfile 从：
...
1. 这份数据是什么类型？
2. 哪些字段具有材料科学语义？
3. 是否包含 composition / structure / trajectory / phonon / volumetric / model result？
4. 哪些列是 target / prediction / uncertainty / class label / probability / material property？
5. 哪些分析从“数据条件”上已经可以执行？
6. 哪些分析无法执行？
7. 缺什么？
8. 是否存在歧义，不能安全自动决定？
9. 这些结论来自什么确定性规则？
...
* capability audit
* gap matrix
* DataProfile inventory
* dataset/ML inventory
* implementation sequence
* 10K-1 next-scope document
...
* 10K-0 尚未 completion-record CI
* 10K-0 尚未 archive
* 10K-0 Gap Matrix 不存在
* 10K-0 result 不存在
* 10K-0 current-head CI 失败
* repository 不 clean
* origin/master != HEAD 且不是合法未推送本阶段 queue transition

则：

**停止 Phase 10K-1 implementation。**

输出：

`BLOCKED_BY_PHASE_10K_0`
...
# 1. Canonical Product Context

项目：

# Material Data Intelligence & Visualization Platform

DataProfile 2.0 在整个系统中的位置：

```text
Raw / Uploaded Material Data
            ↓
Parsing / Canonical Resource
            ↓
Material Data Profile 2.0
            ↓
Deterministic Semantic Recognition
            ↓
Analysis Readiness
            ↓
10K Dataset / ML Products
            ↓
10L Capability-Aware Planner
```

非常重要：

**Phase 10K-1 不实现 Agent Intelligence。**

这一阶段只建立 Agent 未来可以信任的数据事实。
...
* DataProfile schema/model extension
* deterministic material semantic recognition
* semantic column roles
* resource semantic summarization
* analysis-data readiness
* ambiguity reporting
* unsupported/missing requirement reporting
* profile persistence/API serialization
* minimal profile frontend display
* compatibility layer
* profile fixtures
* unit/integration/API/browser tests
* caps
* deterministic hashing/versioning if current architecture requires
* documentation
* evidence
* current-head CI
...
* Dataset Materials Explorer
* dataset dashboards
* space-group histogram product
* dataset comparison product
* duplicate detection product
* anomaly detector product
...
* residual product
* MAE/RMSE/R² tool
* uncertainty calibration product
* confusion matrix
* ROC
* PR
...
* UMAP execution
* clustering
* composition embedding product

### Phase 10L

* capability-aware Planner behavior
* automatic multi-tool selection
* plan repair
...
* Unified Scientific Workspace redesign
...
* status
* Phase 10K-0 commit
* Phase 10K-0 completion-record commit
* Phase 10K-0 archive commit
* Phase 10K-0 exact-head CI
...
当前字段、来源、持久化、API、Planner、frontend。

## Semantic Column Audit
...
10K-1 被正式冻结的 exact scope。

---

# 6. 10K-0 是直接实施依据

以下 prompt 给出的是 **产品与科学边界**。

但实际代码修改必须：

> 以 Phase 10K-0 审计结果为 implementation truth。

如果 10K-0 发现某项已经 READY：

不要重复实现。

如果 10K-0 发现现有架构与下面建议命名不同：
...
# 7. Pre-Implementation Architecture Audit

修改任何实现前，必须输出：

# Phase 10K-1 Pre-Implementation Audit
...
## Principle 1 — Extend Existing Profile

除非 10K-0 明确证明现有 DataProfile 无法安全演进，否则：
...
“Data Profile 2.0” 是产品阶段名称。

不要机械假定 schema 必须命名：

`data_profile.v2`

必须先审计项目现有 contract versioning。

如果已有 schema version：

遵循现有规则。

如果当前 profile 是数据库 JSON 结构且支持 additive evolution：

可以采用兼容性扩展。

如果确实需要 breaking schema：

必须：

* version
* backward compatibility
* fixture migration/adapter
* API compatibility

不得 silent break existing stored profile。

---

# 10. Facts 与 Inference 必须分离

DataProfile 2.0 不应该把所有东西混成一个字典。
...
# 11. Semantic Recognition 不使用 LLM

所有 DataProfile 2.0 classification 必须是：
...
* embedding-based column guessing
* remote service
* arbitrary natural-language model call
...
# 12. Semantic Authority Priority
...
```text
1. Explicit structured metadata / canonical contract
2. Explicit user-declared role already persisted by platform
3. Canonical exact field name
4. Reviewed alias allowlist
5. Bounded structured pattern
6. Unknown
```

如果仓库已有不同但合理 precedence：
...
```text
authority = explicit_metadata
authority = canonical_name
authority = alias_match
authority = bounded_pattern
```
...
* missing/null
* unique count
* min/max where current profile supports
* finite/non-finite state
* preview metadata

但：

不要把 Phase 10K-1 变成完整 statistics engine。
...
属于 Dataset Explorer/tool。

Profile 只保留分析规划需要的轻量事实。

---

# 15. Material Composition Semantics

Profile 应能够在真实输入支持时确定：

* formula column
* composition source
* parsed formula availability
* elements
* chemical-system availability
...
必须基于 10K-0 和现有项目规则冻结。

不得无限添加猜测 alias。

---

# 16. Invalid Formula Handling

存在 formula-like 列不代表所有 row 都合法。
...
# 17. Structure Semantics

不要重新解析已经 canonicalized 的 structure。

优先消费现有 resource metadata。
...
* lattice presence
* periodicity
* dimensionality if authoritative
* volume if defined
* density if defined
* space-group information if already computed/authoritative

注意：

DataProfile 不应为了生成 profile 自动执行昂贵 structure analysis。
...
* RDF
* XRD
* full symmetry workflow

除非当前 profiler 已有 bounded lightweight behavior。

---

# 18. Trajectory Semantics

如果 canonical trajectory 已存在：

Profile 可暴露已有 metadata：

* trajectory present
* frame count
* atom count policy
* species identity
* timestep/time availability
* lattice mode
* wrapped/unwrapped availability
...
通过现有 canonical phonon resource/artifact metadata 暴露：

* phonon bands available
* q-point path
* frequencies
* DOS available
* projected DOS availability
* eigenvectors availability
* imaginary frequencies detectable from existing data

不要在 profile 时运行 phonopy solver。

不要补算缺失数据。

---

# 20. Volumetric Semantics

复用 Phase 10J canonical metadata。

Profile 可表达：

* volumetric data present
* scalar/vector/complex
* grid dimensions
* quantity
* units
* normalization
...
不要重新扫描大体积 binary payload 才生成 profile。

优先使用已有 manifest/header metadata。

---

# 21. Model Result Semantics

这是 Phase 10K-1 的关键新增能力。

必须根据 10K-0 gap 实现可靠识别。
...
实际 field 名称由现有代码风格决定。

---

# 22. Target / Prediction Alias Policy

建立有限、reviewable alias set。
...
不要在 10K-1 实现 model comparison。

但不能把 schema 锁死成一个 prediction 字段。

---

# 24. Multi-Target Dataset
...
不要强行合并成一个 regression pair。

Profile 应：

* 识别多个 candidate tasks
* 或标记 ambiguity
* 或表达多个 semantic groups

具体采用哪种结构：

依据 10K-0 和现有 architecture。

必须具有未来扩展能力。

---

# 25. Uncertainty Semantics

识别未来 Materials ML 需要的：
...
可以标记 uncertainty role。

但不要擅自解释：

“95% confidence interval”。

必须保留 uncertainty kind。
...
但不要把任何 categorical column 都当：

classification target。

需要 semantic naming/explicit metadata。

---

# 28. Probability Columns
...
或平台当前已有类似 convention。

Profile 可以表达：

* probability group
* classes
* normalization/readiness warning
...
# 29. Materials Property Recognition

第一版应该是有限、明确、可维护的 property semantic catalog。

基于 10K-0 actual findings 和现有 capabilities。
...
不要建立一个号称认识“所有材料属性”的开放字典。

---

# 30. Unit Handling

只有输入/contract 明确提供 unit 时：
...
* one column
* zero/one/multiple semantic roles
...
为了支持未来 10K-3，推荐审计是否需要一种轻量 group 表达：
...
# 33. Analysis Readiness

DataProfile 2.0 的重要能力：
...
### Platform Implementation Availability

当前平台有没有对应正式 Tool。

---

# 34. 不要把 Future Tool 写成 Available
...
但 10K-3 尚未实现 regression evaluation tool。

正确表达应类似：

```text
data_readiness:
  regression_evaluation = READY
```

但：

```text
available_platform_analysis:
  regression_evaluation = NOT_IMPLEMENTED_YET
```

或者通过当前 architecture 的 equivalent distinction。

绝对禁止：

因为数据足够，就写：

`ml.regression_evaluation available=true`

如果 Tool 尚不存在。

---

# 35. Analysis Readiness Categories
...
* AMBIGUOUS
* UNSUPPORTED_DATA_KIND

如果现有项目已有 typed status：
...
# 36. Initial Readiness Types

DataProfile 2.0 至少应考虑：
...
# 37. Capability Resolver Architecture

审计最合理的设计位置。

不要把 Tool Registry 业务全部塞进低层 profile model。
...
DataProfile
      ↓
AnalysisReadinessResolver
      ↓
Data readiness
```

如果需要当前平台 availability：
...
Tool Registry
      ↓
Available Capability View
```

这样 future 10L Planner 才能安全消费。

如果现有 architecture 已有 equivalent resolver：
...
# 38. DataProfile 不拥有 Planner Logic

禁止在 profiler 中写：

```python
if y_true and y_pred:
    plan.add("ml.regression")
```
...
Plan selection 留给 10L。

---

# 39. Reasons / Missing Requirements
...
reason:
prediction present but uncertainty column missing
```
...
reason:
multiple label candidates
```

这将直接支持未来 Agent explanation。

---

# 40. Stable Resource / Sample Identity

读取 10K-0 对 sample identity 的结论。

如果 10K-0 将其判定为 10K-1 blocker：

本阶段必须建立最小稳定 identity foundation。
...
resource identity + stable row index

但要明确：

row index identity 只对当前 immutable dataset version 有效。

不要基于可变排序生成身份。

---

# 41. 数据不可变性 / Version Binding

如果 dataset/resource 已有 version/hash：

semantic profile 必须绑定对应版本。
...
复用现有 resource hash/version mechanism。

不要引入第二套 version system。
...
DataProfile 2.0 必须 deterministic。
...
除非 metadata timestamp 本来属于 persistence envelope。

---

# 43. Bounded Execution

Profiler 是 upload/analysis 的基础层。
...
根据 10K-0 recommendation 增加 bounded policy。

---

# 44. Sampling Disclosure
...
不能把 sampled inference 伪装成：

whole dataset certainty。

尤其 formula parseability、semantic validation。
...
需要 typed profile warnings/errors。
...
* incompatible target/prediction length
* invalid probability group

Profile 本身应尽量：

**degrade gracefully。**

只有 resource contract 根本无效时才 hard fail。

---

# 46. Existing Profile Backward Compatibility

必须新增 regression tests 确认：
...
* 仍能 display
* 旧 API consumer 不崩溃
* existing Planner 不因新增字段改变 route
* existing artifacts 不变
* existing stored jobs 不失效
...
# 47. Planner Compatibility Test

因为未来 Planner 会用 Profile 2.0，

但本阶段：

**Planner behavior 必须保持不变。**

必须测试：

相同输入 prompt + existing deterministic planner：

10K-1 前后的 Tool routing 不因新增 profile fields 偷偷改变。

除非只是修复 10K-0 已证明的 profile serialization bug。

如果发现 Planner 自动读取任意新字段导致 route 改变：
...
更新现有 DataProfile API。

不要新增一套平行 endpoint，除非 architecture 明确要求。

API 应能返回：

* existing profile fields
* new material semantics
* semantic columns/groups
* data readiness
* warnings
...
arbitrary object dump。

---

# 49. API Backward Compatibility

如果现有 frontend/clients 依赖 profile JSON：

新增字段优先 additive。

如必须改字段：

提供 compatibility path。
...
y_true → regression target
y_pred → regression prediction
...
Regression evaluation
Data ready / platform phase pending
```
...
# 51. 不要把 Frontend 做成复杂 Dashboard
...
现有 PlannerWorkbench/Resource UI 中：

以最小产品方式显示 Profile 2.0 即可。
...
* proper labels
* screen-reader readable warnings
* mobile responsive
...
Column names、formula strings 等必须作为 text。

---

# 54. Fixtures

建立小而高价值的 canonical fixtures。

至少覆盖：

## Case A — General Material Table
...
* no schema collapse
* multiple prediction representation

---

## Case E — Ambiguous Target

设计一个真实 ambiguity case。
...
## Case G — Invalid Formula

mixed valid/invalid。

---

## Case H — False Positive Defense
...
必须证明不会因为 substring 粗暴误判成 ML/formula semantic。

---

# 55. Resource Fixtures

同时针对已有 resource contract 验证 Profile 2.0 能安全消费：
...
不需要重新运行这些重型 renderer。

只测试 metadata/profile bridging。
...
* empty table
* duplicate column names if platform permits
* all-null semantic candidate
* string y_pred
* incompatible target/prediction
* non-finite numeric values
* multiple target candidates
* invalid probability columns
* no formula
* unsupported file
* malformed metadata
...
# 57. Property-Based / Invariant Tests

如果当前测试体系已有 Hypothesis：

建议加入 bounded invariant。
...
profile generation does not mutate resource

### Stable order

semantic roles deterministic

### False-positive resistance

random unrelated numeric columns ≠ automatically prediction

### Compatibility

old profile fixture remains loadable
...
**不新增任何 scientific Tool Registry tool。**

DataProfile 2.0 是 platform intelligence infrastructure。

不得注册：

```text
profile.detect_semantics
...
# 59. Existing Tool Registry Compatibility
...
不因 profile schema 扩展而失效。

---

# 60. Artifact Contract

DataProfile 是否本身属于 artifact/resource metadata：

以现有 architecture 为准。

不要无故创建：

`material_profile.v2 artifact`

如果现有 profile 已作为 resource metadata。

如果 profile 本来就是正式 artifact：
...
必要时只更新类型/serialization compatibility。

---

# 62. Data Capability 与 Platform Capability
...
但不要硬编码 Phase 名称进 runtime schema，除非项目已有类似设计。

更好的 runtime semantics：

```text
data_readiness = READY
platform_capability = UNAVAILABLE
```

Roadmap phase 只写 docs。

---

# 63. 不允许 Runtime 依赖 Roadmap
...
新增/更新实际适合的 Phase 10K 文档。

建议：

```text
docs/phase10k/
  phase10k1_material_data_profile_2_implementation.md
  phase10k1_semantic_role_contract.md
  phase10k1_analysis_readiness_contract.md
  phase10k1_fixture_matrix.md
  phase10k1_next_scope.md
```
...
# 65. Semantic Role Documentation

必须记录 canonical semantic roles。
...
# 66. Alias Registry Documentation

如果实现 alias allowlist：
...
# 67. Analysis Readiness Documentation

记录：

每项 readiness 的最低数据条件。
...
Composition analysis:
parseable composition/formula

Classification evaluation:
class target + class prediction
```
...
* DataProfile 2.0 implementation
...
* dataset comparison policy → 10K-2
* ML metric authority → 10K-3
* PCA/clustering → 10K-4
* capability-aware planning → 10L
...
* no new tools in 10K-1
* Profile 2.0 provides future tool eligibility facts
* future ML/Dataset tools consume it
...
> DataProfile facts 与 AnalysisReadiness 分离

或：

> deterministic semantic authority hierarchy

且这是新 architectural contract，
...
* uncertainty
* ambiguous case
* one non-table scientific resource

不得只测试 Python function。

---

# 71. Browser Evidence

最小 browser evidence 必须证明：

用户可以看到：

* detected material semantics
* readiness
* warnings
...
如果 repository 当前 browser evidence policy 要求 Firefox/WebKit：
...
# 72. Browser Evidence 不是 10K-5 的替代

10K-1 browser evidence 只证明：

**Profile 2.0 product surface works。**

10K-5 以后仍负责：

Material Intelligence full integration closure。

---

# 73. Performance Evidence
...
不需要开展独立 benchmark Phase。

---

# 74. No External Network

必须证明：

DataProfile semantic detection：
...
未被 profile change 破坏。

不要求重跑所有重型 browser screenshot evidence。

但 unit/integration/service-backed regression 必须通过。
...
代替本阶段 current-head regression。

---

# 78. No New Dependency

本阶段理论上不应该需要新依赖。

semantic detection 应使用当前 stack。

如果真的认为必须加 dependency：
...
```text
Implement material data profile semantics
```

实际遵循 repository commit style。
...
# 80. Implementation-HEAD CI

必须验证 implementation commit exact SHA。
...
# 81. Completion Record

CI success 后才写 Phase 10K-1 completion record。
...
* security
* explicit limits
* deferred 10K-2/3/4/10L

然后提交 completion-record commit。

---

# 82. Completion-Record CI

completion-record exact SHA 再跑 current-head CI。

只有 success 后：

才 archive 10K-1 TASKS block。
...
```text
DataProfile knows the data is eligible for regression analysis.
```

不正确：

```text
DataProfile has evaluated the regression model.
```

---

# 85. 推荐最终 Readiness 示例

这是语义参考，不要求采用相同 JSON：

```json
{
  "data_kind": "materials_table",
  "semantics": {
    "composition": {
...
实际 contract 必须由仓库设计决定。

---

# 86. Phase 10K-1 Final Report Format
...
## 3. Existing Profile Architecture Reused
...
## 24. Whether Allowed to Enter Phase 10K-2
...
* 10K-0 archived
* Profile 2.0 implementation complete
* deterministic semantics complete
* ambiguity policy complete
* analysis readiness complete
* backward compatibility complete
* API evidence complete
* frontend profile evidence complete
* false-positive tests complete
...
Phase 10K-1 只有全部满足才 PASS：

1. 读取并依据真实 10K-0 audit/gap matrix。
2. 没有建立第二套平行 DataProfile。
3. Existing profile compatibility preserved。
4. Material semantics deterministic。
5. 不使用 LLM 识别列。
6. Semantic authority policy 明确。
7. Alias set bounded。
8. Ambiguity 不 silent resolve。
9. formula/composition semantics 完成到 10K-0 scope。
10. material property semantics 完成到批准范围。
11. regression target/prediction semantics 完成。
12. multiple prediction 结构不会被 schema 锁死。
13. uncertainty semantics 完成。
14. classification semantics 按审计范围完成。
15. structure canonical metadata 可以进入 profile。
16. trajectory canonical metadata 可以进入 profile。
17. phonon canonical metadata 可以进入 profile。
18. volumetric canonical metadata 可以进入 profile。
19. Data readiness 与 Platform availability 分离。
20. readiness reason 可解释。
21. sample/resource identity 按审计要求完成。
22. deterministic output。
23. bounded execution。
24. sampling 如存在则明确 disclosure。
25. invalid/ambiguous input typed handling。
26. old profile fixtures 兼容。
27. existing Planner behavior 不改变。
28. existing Tool Registry 不改变。
29. QueueWorkerRuntime semantics 不改变。
30. 没有新增 scientific Tool。
31. 没有新增 dependency。
32. 最小 frontend profile surface 完成。
33. accessibility PASS。
34. API evidence PASS。
35. browser evidence PASS。
36. performance evidence满足当前 caps。
37. no external network。
38. no secrets。
39. full regression满足项目 gate。
40. implementation exact-SHA CI success。
41. completion-record exact-SHA CI success。
42. 10K-1 archived。
43. 10K-2 remains NEXT。
44. origin/master == HEAD。
45. git clean。
...
先读取 Phase 10K-0 的真实结果并输出：

# Phase 10K-1 Entry / Contract Audit

必须明确：

1. 10K-0 是否已经正式 ARCHIVED。
2. 当前 DataProfile 的真实 schema 和 implementation。
3. 10K-0 已确认的 Profile 缺口有哪些。
4. 哪些能力已经 READY，不需要重做。
5. semantic detection 当前在哪里。
6. resource metadata 当前在哪里。
7. sample identity 是否是 10K-1 blocker。
8. analysis readiness 应放在哪一层。
9. 如何保持旧 profile/API/Planner compatibility。
10. 本阶段将修改哪些文件类别。
11. 哪些东西明确留给 10K-2 / 10K-3 / 10K-4 / 10L。
