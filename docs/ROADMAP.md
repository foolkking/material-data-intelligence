...
and typed unavailable identities. Phase 10M-4 is the current reviewer-approved
implementation: an exact application-owned renderer registry, metadata-first
Artifact Gallery, existing scientific viewer adapters, M3 selection reuse, and
bounded heavy-viewer lifecycle are present locally. Its implementation,
completion-record, and queue-archive exact-SHA CI remain open. Report/Recipe
composition and recovery remain reviewer-gated Phase 10M-5+ scope.
